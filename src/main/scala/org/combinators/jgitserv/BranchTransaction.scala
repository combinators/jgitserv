...
sealed trait BranchTransaction { self =>
  ...
  def deleteAllFiles: BranchTransaction = new BranchTransaction {
    ...
    def materialize(inGit: Git): IO[Unit] =
      self.materialize(inGit) *> IO {
        val tree = JGitFileUtils.pathToString(
          JGitFileUtils.canonicalize(inGit.getRepository.getWorkTree)
        )
        val toDelete: Iterator[(File, String)] =
          FileUtils
            ...
            .map(f => {
              val canonicalF =
                JGitFileUtils.pathToString(JGitFileUtils.canonicalize(f))
  ...
  /** Persists the given element to disk and adds it to the index, overwrites existing elements.
    ...
    */
  def persist[E](elem: E, sourceDirectory: Path = Paths.get("."))(
      implicit persistable: Persistable.Aux[E]
    ...
    def materialize(inGit: Git): IO[Unit] =
      self.materialize(inGit) *> IO {
        ...
        val treeString =
          JGitFileUtils.pathToString(JGitFileUtils.canonicalize(tree.toFile))
        val persisted =
          persistable.persistOverwriting(tree.resolve(sourceDirectory), elem)
        val targetPath = tree.resolve(persisted.toPath)
        val targetString = JGitFileUtils.pathToString(
          JGitFileUtils.canonicalize(targetPath.toFile)
        )
  ...
  def persist[E](
      elems: Seq[E]
  )(implicit persistable: Persistable.Aux[E]): BranchTransaction =
    elems.foldLeft(this)((branch, elem) => branch.persist(elem))

  /** Commits the current index state to the branch. */
  def commit(message: String = "", signed: Boolean = false): BranchTransaction =
